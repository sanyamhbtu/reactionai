...
export function ReactionTool() {
  ...
  const [structure, setStructure] = useState<{ smiles: string; molfile?: string } | null>(null)
  ...
  async function predict() {
    ...
    try {
      const response = await fetch('/api/predict', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ input: structure ? `${input.trim() || 'Explain this molecule'}\nDrawn structure SMILES: ${structure.smiles}` : input, mode, learningMode }) })
